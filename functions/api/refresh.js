...
function naturalRating(apiRating, index){
  const r = parseFloat(apiRating);
  if(r && r > 0 && r <= 5) return Math.round(r*10)/10;
  const seed = (index * 7 + 13) % 100;
  const rating = 3.8 + (seed / 100) * 1.1;
...
function pickCat(term){
  const electronics=['earbuds','bluetooth speaker','smartwatch','laptop stand','phone case','usb hub','webcam','mechanical keyboard','gaming mouse','portable charger','led desk lamp','smart plug','ring light','tablet stand','qi charger','hdmi cable','monitor light bar','cable organizer'];
  const home=['air fryer','coffee maker','robot vacuum','instant pot','blender','mattress topper','throw pillow','scented candle','storage organizer','shower curtain','bath towel','kitchen knife','cast iron pan','ice cube tray','dish rack','silicone spatula','electric kettle'];
  const fashion=['running shoes','gym leggings','baseball cap','sunglasses','leather wallet','tote bag','winter scarf','compression socks','sports bra','hoodie','minimalist watch','canvas sneakers','crossbody bag'];
  const beauty=['skincare lotion','face serum','vitamin c cream','hair mask','electric toothbrush','jade roller','sunscreen','lip gloss','eyelash curler','nail kit','facial cleanser','eye cream','hair growth oil','makeup brush set'];
  const sports=['resistance bands','yoga mat','water bottle','jump rope','foam roller','gym gloves','protein shaker','ankle weights','pull up bar','knee sleeve'];
...
export async function onRequest(context){
  const {env}=context;
  if(!env.RAPIDAPI_KEY) return new Response(JSON.stringify({error:'Missing RAPIDAPI_KEY'}),{status:500});
  if(!env.DB) return new Response(JSON.stringify({error:'Missing DB binding'}),{status:500});
  try{
    const meta=await env.DB.prepare('SELECT value FROM catalog_meta WHERE key=?').bind('run_index').first();
    let runIndex=parseInt(meta?.value||'0');
    const terms=[];
    for(let i=0;i<BATCH_SIZE;i++) terms.push(SEARCH_TERMS[(runIndex+i)%SEARCH_TERMS.length]);
    const newIndex=(runIndex+BATCH_SIZE)%SEARCH_TERMS.length;
    await env.DB.prepare('INSERT OR REPLACE INTO catalog_meta(key,value) VALUES(?,?)').bind('run_index',String(newIndex)).run();
    const summary={};
    for(const term of terms){
      ...
      for(let i=0;i<results.length;i++){
        ...
        const rating=naturalRating(item.averageStarRate,i);
        const reviews=parseInt(item.sales||0)||Math.floor(Math.random()*8000)+200;
        try{
          await env.DB.prepare(`INSERT OR IGNORE INTO products(id,name,cat,icon,price,rating,reviews,hot,desc,thumbnail,stock,source_url,source) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,'aliexpress')`).bind(
            id,item.title||term,pickCat(term),pickEmoji(term),price,rating,reviews,
            i<3?1:0,'Shipped direct to your door.',thumbnail,Math.floor(Math.random()*40)+5,aliUrl
          ).run();
          inserted++;
        }catch(e){}
      }
    ...
    return new Response(JSON.stringify({success:true,batch:terms,summary,nextIndex:newIndex}),{headers:{'Content-Type':'application/json'}});
  }catch(err){
